...
// Patch fs.readFileSync to locate font files in the correct directory
...
(fs as any).readFileSync = function (...args: any[]) {
  ...
  if (typeof filePath === 'string' && filePath.toLowerCase().endsWith('.afm')) {
    ...
    if (standardFontFiles.includes(fileName) && fs.existsSync(fixedPath)) {
      return (originalReadFileSync as any).apply(this, [fixedPath, ...args.slice(1)]);
    }
    const altPath = path.join(pdfkitDir, 'data', fileName);
    if (standardFontFiles.includes(fileName) && fs.existsSync(altPath)) {
      return (originalReadFileSync as any).apply(this, [altPath, ...args.slice(1)]);
    }
  }
  return (originalReadFileSync as any).apply(this, args);
};

// Pre-load standard font paths (kept for compatibility)
...
export function drawClientDetails(
  doc: PDFDocumentInstance,
  client: any,
  ...
  const margin = doc.page.margins.left;
  const leftWidth = 280;
  const rightWidth = doc.page.width - margin * 2 - leftWidth - 10;
  const startY = doc.y;
...
export function drawItemsTable(doc: PDFDocumentInstance, items: any[]) {
  const margin = doc.page.margins.left;
  ...
  const headerAligns: ('left' | 'center' | 'right')[] = ['left', 'left', 'center', 'right', 'right'];
...
export function drawTotals(doc: PDFDocumentInstance, subtotal: number, total: number) {
  const boxWidth = 240;
  const boxX = doc.page.margins.left;  // ← alineado a la izquierda
  const boxY = doc.y;
  const leftPad = 14;
  ...
  // Subtotal: etiqueta + valor en la misma línea
  ...
  // Total: etiqueta + valor en la misma línea, con negrita
  ...
  // Dejar espacio para el siguiente bloque
  doc.y = boxY + 90;
}

export function drawFooter(doc: PDFDocumentInstance) {
  const margin = doc.page.margins.left;
